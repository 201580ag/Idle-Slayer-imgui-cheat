...
HRESULT __stdcall hkPresent(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	if (globals::show)
	{
		...
		ImGui::Text("sulfate.wtf | %s", std::ctime(&end_time));

		ImGui::End();

		ImGui::SetNextWindowSize(ImVec2(900, 450), ImGuiCond_Once);
		ImGui::Begin("IDLE Slayer Cheat Menu", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
		{ImGui::SameLine();
		...
		if (globals::tab == 0)
		{
			...
			{
				*(double*)mem::FindAddress(moduleBase + 0x01F1BFF0, { 0x40, 0xB8, 0x0, 0x20 }) = globals::coins;
			}

			if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
			{
				ImGui::SetTooltip("Modifys Coin value to the amount.");
			}
		}
		if (globals::tab == 1)
		{
